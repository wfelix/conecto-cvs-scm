...
export class CvsSourceControl implements Disposable {
  ...
  constructor(
    ...
    this.stagedResources = this.cvsScm.createResourceGroup(
      "stagingTree",
      "Staged Changes"
    );
    this.changedResources = this.cvsScm.createResourceGroup(
      "changeTree",
      "Changes"
    );
    this.repositoryResources = this.cvsScm.createResourceGroup(
      "repositoryTree",
      "Repository Changes"
    );
    this.conflictResources = this.cvsScm.createResourceGroup(
      "conflictTree",
      "Conflicts"
    );
    this.unknownResources = this.cvsScm.createResourceGroup(
      "untrackedTree",
      "Untracked"
    );
    ...
    const fileSystemWatcher = workspace.createFileSystemWatcher(
      new RelativePattern(this.workspacefolder, "**/*")
    );
    ...
    context.subscriptions.push(
      window.onDidChangeActiveTextEditor(
        (textEditor) => this.updateStatusBarItem(textEditor),
        context.subscriptions
      )
    );
  ...
  async updateStatusBarItem(textEditor: TextEditor | undefined): Promise<void> {
    if (
      textEditor &&
      dirname(textEditor.document.uri.fsPath).includes(
  ...
  refreshScm(): void {
    ...
    this.cvsRepository.getChangesSourceFiles().forEach((element) => {
      ...
      if (element.state === SourceFileState.modified) {
        ...
        const left = this.cvsRepository.provideOriginalResource!(
          element.uri,
          token.token
        );
        ...
        const command: Command = {
          ...
          arguments: [
            left,
            right,
            `${basename(element.uri.fsPath)} (${this.changedResources.label})`,
          ],
        ...
        const token = new CancellationTokenSource();
        let left = this.cvsRepository.provideOriginalResource!(
          element.uri,
          token.token
        );
        let right = "";
        ...
        const command: Command = {
          ...
          arguments: [
            left,
            right,
            `${basename(element.uri.fsPath)} (${
              this.repositoryResources.label
            })`,
          ],
          tooltip: "View remote changes",
        };
    ...
    this.cvsDocumentContentProvider.updated(
  ...
  async commitAll(consulta?: string): Promise<void> {
    ...
    this.stagedResources.resourceStates.forEach((element) => {
      files = files.concat(
        workspace.asRelativePath(element.resourceUri, false) + " "
      );
    });
  ...
  async removeFileFromCvs(uri: Uri): Promise<void> {
    await runCvsCmd(
      `cvs remove -f ${basename(uri.fsPath)}`,
      dirname(uri.fsPath)
    );
  }
  ...
  async revertFile(uri: Uri): Promise<void> {
    this.unstageFile(uri, false); // in case staged
    await runCvsCmd(
      `cvs update -C ${basename(uri.fsPath)}`,
      dirname(uri.fsPath)
    );
  }
  ...
  async undoAdd(_uri: Uri): Promise<void> {
    ...
    const entries = await this.readCvsEntries(
      dirname(_uri.fsPath) + "/CVS/Entries"
    );
    ...
    await this.writeCvsEntries(
      dirname(_uri.fsPath) + "/CVS/Entries.out",
      newEntries
    );
    await fsPromises.rename(
      dirname(_uri.fsPath) + "/CVS/Entries",
      dirname(_uri.fsPath) + "/CVS/Entries.bak"
    );
    await fsPromises.rename(
      dirname(_uri.fsPath) + "/CVS/Entries.out",
      dirname(_uri.fsPath) + "/CVS/Entries"
  ...
  async ignoreFolder(uri: Uri): Promise<void> {
    await this.configManager.updateIgnoreFolders(
